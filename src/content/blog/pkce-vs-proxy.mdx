...
getting a cookie in return. This cookie is then sent with every request so
that the server knows who is making the request. However, when services
wanted to interact with each other on your behalf other systems authorization
systems where born. One such system is OAuth which uses token-based
authentication instead of cookies.
...
Unfortunately, there are a couple of inherent security risks with this setup.
...
In order to protect credentials sent between services and what the service is
allowed to do with those credentials, some initial trust needs to be
bootstrapped and protected ahead of time. This is done by combining two
different components to ensure that the recipient of a temporary authorization
code is, in fact, the one it is intended for. If you have heard of two-factor
authentication(2FA), it is like that but for web services.
