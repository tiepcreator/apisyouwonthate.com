---
title: "Annotations, DSLs, and GUIs for API Descriptions"
subtitle: >-
  Code-first and Design-first share a lot of the same problems and solutions,
  but there have been huge changes in the tooling ecosystem to how annotations,
  domain-specific languages, and editors work for things like OpenAPI and
  GraphQL. Let's see if you change your mind on which of these solutions is
  right for you.
date: "2020-03-30"
...
Some API developers use API descriptions like [OpenAPI](https://www.openapis.org/), [JSON Schema](https://json-schema.org/), or [GraphQL Schemas](https://graphql.org/learn/schema/) to plan the interface of an API before building it, which is known as the "API design first" workflow. Others build the API then generate (or manually produce) API descriptions later, which is the "code-first" workflow. How do people actually create these schemas, and what are the pros and cons of these approaches?

Many of these type systems involve writing out a bunch of special keywords in YAML or JSON; and neither the design first or code first crew enjoy writing thousands of lines of that by hand. For example, just to say something is a list of integers: 
...
A few approaches popped up to avoid folks needing to write all that out, as designing an API with 100 lines like that will get you two at least one keyboard (maybe two if it's a Macbook). Let's look at these alternatives, and see how they fit into design-first, code-first, and how they can be used to achieve the ultimate goal for API designers: one source of truth for API descriptions, mocks, documentation, and more.
...
<figure>
  <img src="../../images/posts/annotations-dsl-editors/editor.png" />
...
GraphQL fans can check out these:
...
<figure>
  <img src="../../images/posts/annotations-dsl-editors/explorer.png" />
