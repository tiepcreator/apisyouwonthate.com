...
coverImage: partials-compromise-between-customization-and-cacheability/cover.png
type: blog
...
fieldsets](https://philsturgeon.uk/api/2017/08/13/a-happy-compromise-between-customization-and-cacheability/jsonapi.org/format/#fetching-sparse-fieldsets)
(tl:dr; send `?fields=foo,bar` to just get foo and bar).

This is common and popular for two reasons:

1. It takes fractionally less time to download a smaller response over the
network
2. Some of those fields might be computed on request, and as such are
slow

Specifications like [OData](http://www.odata.org/) and
...
This is a massive shame! [Varnish](http://varnish-software.com/),
[Squid](http://www.squid-cache.org/), [Fastly](https://www.fastly.com/), etc.,
...
specific endpoints (`GET /turtles/mobile`, `GET /turtles/ios`, `GET
/turtles/ios-random-usecase-tuesday5`. That's no ideal (cache missing all over
...
you're stuck with _all_ the fields, including whatever computed values are there
...
Caching](https://philsturgeon.uk/api/2017/01/26/graphql-vs-rest-caching/),
...
[serialization](https://philsturgeon.uk/api/2015/05/30/serializing-api-output/)
that comes with it.
...
handshakes = slow"](/blog/lets-stop-building-apis-around-a-network-hack).
