...
diagrammed by Martin Fowler in his article introducing [the Richardson Maturity
Model](https://www.martinfowler.com/articles/richardsonMaturityModel.html)
concept. The concept is named after Leonard Richardson and taken from his talk
about [API
...
about the word "maturity" being used. This all makes it seem like REST APIs are
amazeballs and everything else is stupid.
...
Types. Some folks might be working with POX/POJO uf the decision makers in
...
So with the "holier than though" concern out the way, let's take a look at my attempt to update
...
and try again. With a GET there are barely any downsides here, because it is an idempotent
...
it a bit more HTTPish. It's not a REST bridge as some people all it, because it's missing
this next layer...
...
The most basic level is shoving links into the response body (thanks Resources!)
...
it's because Hypermedia Controls make it a REST API, its a lot of the point.
...
Anyway, APIs don’t always need HATEOAS, or any of these levels. They’re extra
layers of abstraction which help you interact less with other developers on the
phone or via email.

Baking it all into the message can add extra data which you might not want to
keep sending around. For example, full-stack developers often think REST is a
waste of time because they are just trying to query the database and get that
information to the presentation layer. They do not need to bake cache controls
into the message itself because they can just set the caching in the client
application which is probably open in another window on their machine. They know
when to use retries or not, because they know their application codes off by
heart, so who cares about leaning on HTTP semantics for that.
...
Check out our article _[Picking the Right API Paradigm](/blog/picking-the-right-api-paradigm)_
to see when you might want to consider using REST, and when you should use something else.