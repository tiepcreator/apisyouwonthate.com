...
Regardless of the [type of API](https://blog.apisyouwonthate.com/picking-the-right-api-paradigm-d476f1a622e8) you are interacting with, and the [approach they take to versioning](https://blog.apisyouwonthate.com/api-versioning-has-no-right-way-f3c75457c0b7), there are no absolute promises the team in charge will deprecate things carefully. Some API providers [act like jerks](https://groups.google.com/forum/#!topic/strava-api/_HW3UN4Mxt4) and just arbitrarily change things, remove things, and break stuff. Closing APIs down with no warning (or poor warning) is utterly disgraceful behavior, but it does happen.
...
This low-tech approach has no real solutions I can think of, other than… subscribe to the api providers engineering blog on RSS, and — so long as the RSS reader you’re using doesn’t get shut down — you might notice the article.
...
![The old Kosciuszko bridge ran side by side with its replacement, until it was time to get rid of the old one. Sadly they didn’t blow it up as planned, I had beers, binoculars and a camp chair all set up… AP Photo/Mark Lennihan](https://cdn-images-1.medium.com/max/800/0*FhDpOM-Mo46zEutZ.jpg)
The old Kosciuszko bridge ran side by side with its replacement, until it was time to get rid of the old one. Sadly they didn’t blow it up as planned, I had beers, binoculars and a camp chair all set up… AP Photo/Mark Lennihan
...
[Sunset is on GitHub](https://github.com/dret/I-D/tree/master/sunset-header), and the issue tracker is used for discussion around it’s development. I’ve been trying to get folks using this. Some API gateways like [Tyk](http://tyk.io/) are [building in support](https://github.com/TykTechnologies/tyk/issues/1626), and it’s going to become far more wide-spread over time. If you make an implementation for your favorite HTTP client please get in touch in the comments.
...
optional int32 old\_field = 6 \[deprecated=true\];
...
Seems like they don’t have an option for deprecating the whole message, but theoretically you can just mark _a_nything as deprecated. The implementations won’t actually do anything about it, but you can probably hack something into something or send some PRs to get that done.
...
If you work for a company that considers itself an API provider, what is your approach to letting folks know something is going away? Do you have a support team who reach out? Is there another approach I missed?

Get in touch in the comments. I’d love to hear from folks who have been handling deprecations in GraphQL and protobuf successfully.