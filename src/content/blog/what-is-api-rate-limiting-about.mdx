...
Often the rate limit will be associated to an “API key” or “access token”, and our friends over at [Nordic APIs](https://nordicapis.com/) very nicely explain some other rate limiting strategies:
...
Ruby users who are already using tools like Sidekiq can add plugins like [Sidekiq::Throttled](https://github.com/sensortower/sidekiq-throttled), or pay for [Sidekiq Enterprise](https://github.com/mperham/sidekiq/wiki/Ent-Rate-Limiting), to get rate limiting functionality. Worth every penny in my books.
...
![[http.cat](https://http.cat/) is an amazing resource](https://cdn-images-1.medium.com/max/800/1*esMflK7UItD8mWn4UbGedg.jpeg)
[http.cat](https://http.cat/) is an amazing resource
...
![[http.cat](https://http.cat/) shows us how a cat can enhance their calm](https://cdn-images-1.medium.com/max/800/1*FQyjo2WDsNPOV5vWaYQaTg.jpeg)
[http.cat](https://http.cat/) shows us how a cat can enhance their calm
...
curl -i [https://api.github.com/users/octocat](https://api.github.com/users/octocat)
HTTP/1.1 200 OK
X-RateLimit-Limit: 60
...
**_Warning:_** _Make sure your sleep does not block your background processes from processing other jobs. This can happen in languages where sleep sleeps the whole process, and that process is running multiple types job on the same thread. Don’t back up your whole system with an overzealous sleep!_

Faraday, a ruby gem I work with often, is [now aware of](https://github.com/lostisland/faraday/pull/773) `[Retry-After](https://github.com/lostisland/faraday/pull/773)`. It uses the value to help calculate the interval between retry requests. This can be useful for anyone considering implementing rate limiting detection code, even if you aren’t a Ruby fan.

To learn _how_ to implement rate limiting, Google it! [Nginx can help you](https://www.nginx.com/blog/rate-limiting-nginx/), API Management gateways like [Kong](https://getkong.org/plugins/rate-limiting/) and [Tyk](https://tyk.io/docs/control-limit-traffic/rate-limiting/) do a great job, or you can try to implement it at the application level for aaaaallllll of your APIs, which is a bit of a pain in the butt. Some frameworks like [Laravel (PHP)](https://laravel.com/) support [basic rate limiting out of the box](https://laravel-news.com/laravel-5-6-dynamic-rate-limiting), which is pretty darn cool.

_This is another excerpt from the book_ [_Surviving Other People’s Web APIs_](https://leanpub.com/surviving-other-peoples-web-apis)_, which is still being written._ [_Preorder it from LeanPub_](https://leanpub.com/surviving-other-peoples-web-apis) _now to support its development, and to keep these articles coming!_

![](https://cdn-images-1.medium.com/max/800/1*F6RdutVwjCeeGtr4jPyIKA.jpeg)