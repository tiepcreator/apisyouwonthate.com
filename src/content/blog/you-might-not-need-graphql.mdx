...
**Note**: This article uses the term [REST](https://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm) as defined in Roy Fielding’s dissertation, RESTish to mean it’s somewhere on the [Richardson Maturity Model](https://apisyouwonthate.com/blog/rest-and-hypermedia-in-2019) but didn’t make it to the top, and endpoint-based APIs to mean any REST/RESTish/RPC/etc. API that uses endpoints instead of POSTing to a single `/graphql` endpoint.
