---
title: Server-Side Validation with API Specifications
subtitle: >-
  Why waste a bunch of time duplicating validation rules your application code
...
out if the data being set to the API is any good or not. Validation can happen
...
for "either field A or B should be set, but not both, and if B is set, we C should be too." sorts
of thing.
...
If you are already doing validation in the controller then this will be a lot nicer, and if
you are doing validation in your model you can delete a whole bunch of noise. If you did not have
validation before, then using this approach means you don't need to start writing it. Win win win!
...
Seeing as OpenAPI will say "this schema should be used for this combination or HTTP Method and Path" you do not
...
Once this middleware is registered with the framework, your controller code
won't even bother getting invoked if the request coming in is invalid. The
framework middleware is able to look at the request, compare it to the API
specs, and reject it with an error format (hopefully something like [RFC
...
- **Node.js:** [express-openapi-validate](https://github.com/Hilzu/express-openapi-validate)
...
They have good gif game too.

<img src="https://i0.wp.com/gifrific.com/wp-content/uploads/2017/11/you-shall-not-pass-gandalf-lotr.gif" alt="You Shall Not Pass!" data-canonical-src="" style="max-width:100%;"/>

Sadly it's only for OpenAPI v2.0.
