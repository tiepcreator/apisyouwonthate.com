...
subtitle: 'Part of my day job is trying to get folks to write API documentation, so the hordes of new developers joining the company on a weekly basis…'
...
```json
{
`"$schema": "http://json-schema.org/draft-06/schema#",` "title": "Foo",
"type": "object",
"properties": {
"id": {
"readOnly": true,
...
Armed with a bunch of reusable contracts, the only thing to do is check if those contracts match the code. Ruby users can do this with [json_matchers](https://github.com/thoughtbot/json_matchers) by Thoughtbot.
