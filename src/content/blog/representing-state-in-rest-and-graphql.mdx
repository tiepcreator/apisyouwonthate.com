...
subtitle: 'Learn how an API can become a state machine, with different actions appearing for resources in different states.'
date: '2017-08-12T14:35:16.936Z'
...
```
{  "data": {    "type": "invoice",    "id": "093b941d",    "attributes": {      "published_at": "2017-06-15 12:31:01Z",      "sent_at": "2017-06-15 12:34:29Z",      "paid_at": null    }  }}
```
...
```
if (model.paid_at) {  status = 'paid';
```

```
} else if (model.sent_at) {  status = 'sent';} else if (model.published_at) {  status = 'published';} else {  status = 'draft';}
```
...
```
{  "data": {    "type": "invoice",    "id": "093b941d",    "attributes": {      "published_at": "2017-06-15 12:31:01Z",      "sent_at": "2017-06-15 12:34:29Z",      "paid_at": "2017-06-16 09:05:00Z",      "payment_received_at": null    }  }}
```
...
Inferring states from dates or other arbitrary fields is awful, and it _always_ goes wrong.

I noticed a similar issue with Postmates Fleet saying “Waiting on verification of your profile image” on the mobile app, and the web app said “Please upload a profile image”. If I dug into the code, I’d bet they were looking for an “image verified” switch of some sort, and the web app just happened to notice a field that the mobile app didn’t.
...
How about the “what to do next” bit? Well, this is exactly what
...
```
{  "data": {    "type": "invoice",    "id": "093b941d",    "attributes": {      "created_at": "2017-06-15 12:31:01Z",      "sent_at": "2017-06-15 12:34:29Z",      "paid_at": "2017-06-16 09:05:00Z",      "payment_received_at": null,      "status": "published"    }  },  "links": {    "pay": "https://api.acme.com/invoices/093b941d/payment_attempts"  }}
```
...
```
{  "schema": "http://api.acme.com/schemas/invoice/093b941d"  "data": {
```
...
```
"links": {  "pay": {    "href": "https://api.acme.com/invoices/093b941d/payment_attempts"    "meta": {      "method": "POST",      "type": "application/json"    }  }}
```
...
_Thanks to the small army of people who battled through my nonsense first drafts in an effort to make me sound intelligent. You know who you are, but especially_ [_@dstockto_](http://twitter.com/dstockto)_,_ [_@glasnt_](http://twitter.com/glasnt)_, and_ [_@mwop_](http://twitter.com/mwop)_._
