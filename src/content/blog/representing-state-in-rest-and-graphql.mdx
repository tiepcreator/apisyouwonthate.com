...
date: '2017-08-12'
coverImage: representing-state-in-rest-and-graphql/cover.png
type: blog
...
[Statesman](https://github.com/gocardless/statesman) is super simple, and
[AASM](https://github.com/aasm/aasm) is cool.
...
Sometimes people make [homegrown solutions](http://zacstewart.com/2012/04/14/http-options-method.html), and some folks leverage tools like [JSON Schema](https://json-schema.org/) for the fields part.

#### **Hyper Schema**

Another approach is [JSON Hyper-Schema](https://json-schema.org/latest/json-schema-hypermedia.html), which is a draft spec. Is essentially an extension to JSON Schema, which adds `links` keywords! Instead of making your own `OPTIONS` metadata and linking to JSON Schema, this Hyper-Schema could be the entire OPTIONS response!
...
Instead of two requests, another approach is combining the metadata with the response document. Our example so far has been using [JSON-API](https://jsonapi.org//), so to continue using that:
...
This isn’t really part of the JSON-API specification, but [it is valid](https://jsonapi.org//format/#document-links). Inventing your own standards and conventions can be a pain in the ass, so maybe don’t bother. There’s a standard for that: [Siren](https://github.com/kevinswiber/siren).
...
_Thanks to the small army of people who battled through my nonsense first drafts in an effort to make me sound intelligent. You know who you are, but especially_ [_@dstockto_](https://twitter.com/dstockto)_,_ [_@glasnt_](https://twitter.com/glasnt)_, and_ [_@mwop_](https://twitter.com/mwop)_._
