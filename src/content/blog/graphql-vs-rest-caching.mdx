...
Recently I wrote [GraphQL vs REST: Overview](https://blog.apisyouwonthate.com/api/2017/01/24/graphql-vs-rest-overview/), giving a hype-free outline of the differences between [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) and [GraphQL](http://graphql.org/). One section that would not have fit into that already lengthy article was caching, so I thought I’d fire that out next.
...
![A request being returned early by a varnish server. — book.varnish-software.com](https://cdn-images-1.medium.com/max/800/1*HVrgB2mX7EC8tz523pkxaA.png)
A request being returned early by a varnish server. — book.varnish-software.com![A request failing to find a match (a.k.a cache miss), and being passed on to the API server to fulfill. — book.varnish-software.com](https://cdn-images-1.medium.com/max/800/1*43stxceFqvz0WDO_2KmOVg.png)
...
Something I talked about a lot in the [GraphQL vs REST: Overview](https://blog.apisyouwonthate.com/api/2017/01/24/graphql-vs-rest-overview/) article is that REST is often considered to force the return of all the data, whilst GraphQL makes you specify the fields you would like returned. I explained the fact that REST APIs can absolutely do the same thing, but this can reduce the likelihood of a cache hit.
